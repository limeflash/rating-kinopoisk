...
Что можно настроить на `/configure`:
- API-ключ Кинопоиска прямо в ссылке manifest (персональная установка),
- имя стрима (`Stream Name`),
- формат рейтинга (`8.1/10` или `8.1`),
- формат голосов (`45,123` или `45.1K`),
- показывать/скрывать голоса,
- multi-line/single-line формат вывода,
- показывать рейтинг только для фильмов или только для сериалов,
- live preview блока как это будет выглядеть в Stremio.

Важно:
- если укажешь API-ключ на `/configure`, он попадет в закодированный URL manifest;
- такой URL лучше не публиковать и не шарить публично.
